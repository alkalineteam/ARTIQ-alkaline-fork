...
class clock_transition_scan(EnvExperiment):
    ...
    #parameters for the sequence that need to be different each time
    #stepping_aom_freq = when scanning this should be equal to scan_frequency_value[j] 
    #param_1 = this will be the main parameter that we are using e.g. bias field, probe power etc.
    ...
    def run(self):
        ...
        for j in range(int32(cycles)):        
            self.run_sequence(j,
                16*dB,    #Here the parameter we are changing is the probe power
        ...
        probe_shift = center_frequency_2 - center_frequency_1
        print(probe_shift)
